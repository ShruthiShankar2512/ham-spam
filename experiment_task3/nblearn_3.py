...
def remove_numeric_words(text):
    re.sub(r'\w*\d\w*', '', text).strip()
    return text


def correct_spelling(tokenized_text):
    spell = Speller(lang='en')
    corrected_text = [spell(w) for w in tokenized_text]
...
def get_clean_data(text_list):
    ...
    clean_data = []
    for t in text_list:
        t = remove_numeric_words(t)
        tokenized_text = tokenize(t)
        no_stop_words_text = remove_stop_words(tokenized_text)

        stemmed_text = [ps.stem(w) for w in no_stop_words_text]
        clean_data.append(stemmed_text)
    return clean_data
...
def main():
	...
	start_reading = timer()
	data_dict = read_data(path_to_data)
